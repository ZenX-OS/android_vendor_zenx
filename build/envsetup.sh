...
cat <<EOF
Additional Zenx functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- zenxgerrit:   A Git wrapper that fetches/pushes patch from/to Zenx Gerrit Review.
- zenxrebase:   Rebase a Gerrit change and push it again.
- zenxremote:   Add git remote for Zenx Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for Zenx Github.
...
function makerecipe() {
    ...
    then
        pwd
        zenxremote
        git push zenx HEAD:refs/heads/'$1'
    fi
    '
}
...
function zenxgerrit() {
    ...
    local user=`git config --get review.review.zenxroms.com.username`
...
function zenxrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      zenxrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "https://review.zenxroms.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
